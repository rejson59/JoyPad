...
import { CanvasRound, COLORS, type Racer, type RoundHud, type RoundResult } from './runtime';
...
export function ArcadeGameView({ id, onExit, remote }: { id: ArcadeId; onExit: () => void; remote: RemoteEvent | null }) {
  ...
  const [primary, setPrimary] = useState(id === 'race' || id === 'snake' ? 1 : 0);
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    const config = {
      players: participants, padInputs: padHost.inputs, primary, secondary,
      onHud: (next: RoundHud) => {
        if (cancelled) return;
        setHud(next);
    ...
    // Każda runda otrzymuje migawkę ustawień i graczy w momencie startu.
    ...
    <div className={`arcade-page arcade-${id} relative min-h-screen overflow-hidden text-white`} style={cssVars}>
      ...
      <div className="relative mx-auto max-w-[1280px] px-4 pb-16 sm:px-7">
        ...
        <section className="arcade-hero relative mt-7 flex min-h-[390px] items-end overflow-hidden rounded-[30px] border border-white/15 p-6 sm:min-h-[440px] sm:p-10">
          ...
          <div className="pointer-events-none absolute inset-0 bg-[linear-gradient(90deg,rgba(5,9,18,.94)_0%,rgba(5,9,18,.75)_45%,rgba(5,9,18,.12)_100%),linear-gradient(0deg,rgba(5,9,18,.9),transparent_70%)]" />
          <div className="relative z-10 max-w-[650px]"><div className="joy-kicker" style={{ color: info.accent }}>◆ {info.eyebrow} / {info.genre.toUpperCase()}</div>
            ...
            <div className="mt-6 flex flex-wrap items-center gap-3"><button onClick={start} className="arcade-start flex items-center gap-3 rounded-xl px-6 py-3.5 text-sm font-black text-[#101117] transition hover:-translate-y-0.5 hover:brightness-110" style={{ background: info.accent, boxShadow: `0 16px 45px ${info.accent}50` }}><Play size={18} fill="currentColor" /> ROZPOCZNIJ GRĘ <ArrowRight size={17} /></button><span className="rounded-full border border-white/20 bg-black/40 px-3 py-2 text-xs text-white/80">{info.players}</span></div>
          </div>
        ...
        <div className="mt-6 grid gap-4 lg:grid-cols-[minmax(0,1.45fr)_minmax(280px,1fr)]">
          <div className="arcade-panel rounded-[26px] p-5 sm:p-6"><div className="mb-4 flex items-center gap-2 font-bold"><Settings2 size={18} style={{ color: info.accent }} /> Ustawienia rozgrywki</div>
            <div className="grid gap-3 sm:grid-cols-2"><OptionStepper title={rules.primaryLabel} value={rules.primary[primary]} accent={info.accent} change={stepPrimary} help="Pilot: ← / →" /><OptionStepper title={rules.secondaryLabel} value={rules.secondary[secondary]} accent={info.accent} change={stepSecondary} help="Pilot: ↑ / ↓" /></div>
  ...
  return (
    <div className={`arcade-page arcade-${id} flex h-screen min-h-[360px] flex-col overflow-hidden text-white`} style={cssVars}>
      <div className="z-20 flex shrink-0 items-center justify-between gap-2 border-b border-white/10 bg-[#0b1020]/95 px-3 py-2 sm:px-5"><div className="flex min-w-0 items-center gap-2 sm:gap-4"><div className="arcade-title truncate text-sm font-bold sm:text-lg" style={{ color: info.accent }}>{info.title}</div><span className="hidden rounded-full border border-white/10 px-2 py-1 text-[10px] font-bold text-slate-300 sm:block">{hud?.status || 'ŁADOWANIE ARENY…'}</span></div>
        ...
        <div className="flex items-center gap-1"><button onClick={mute} title={muted ? 'Włącz dźwięk' : 'Wycisz'} className="arcade-icon">{muted ? <VolumeX size={17} /> : <Volume2 size={17} />}</button><button onClick={() => setShowPads(true)} title="Pady i kod pokoju" className="arcade-icon"><Smartphone size={17} /></button><button onClick={() => round.current?.togglePause()} title="Pauza (P)" className="arcade-icon"><Pause size={17} /></button><button onClick={() => setStage('menu')} title="Zakończ rundę" className="arcade-icon text-amber-300"><Home size={17} /></button><button onClick={onExit} title="Wróć do JoyPad" className="arcade-icon text-violet-300"><Gamepad2 size={17} /></button></div>
      </div>
      <div className="relative min-h-0 flex-1 bg-[#060a16]"><canvas ref={canvas} className="absolute inset-0 h-full w-full" aria-label={`Arena gry ${info.title}`} />
