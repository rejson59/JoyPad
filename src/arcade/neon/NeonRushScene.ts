...
export class NeonRushScene {
  ...
  constructor(
    ...
    this.renderer.domElement.setAttribute('aria-label', 'Neon Circuit 3D');
  ...
  private readInput(slot = 0) {
    ...
    if (external && (slot > 0 || hasExternal)) {
      ...
      const wasPressed = this.externalAction.get(slot) ?? false;
      if (external.action && !wasPressed && this.phase === 'race') car.startBoost(0.16, 4);
      if (external.action && !wasPressed) this.settings.onFx?.(this.settings.playerSlots?.[slot] ?? slot, 'fire');
      this.externalAction.set(slot, external.action);
      return;
    }
    if (slot > 0) {
    ...
    c.drift = drift;
    if (item && !this.itemPressed && this.phase === 'race') this.items.use(car, this.karts);
